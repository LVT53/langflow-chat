...
**Generated-file debug note**: when debugging missing chat files, the current server-side log prefixes are `[FILE_GENERATE]` for the sandbox endpoint, `[CHAT_FILES]` for persistence/listing, `[CHAT_STREAM]` for stream-end payloads and `generate_file` tool markers, and `[CONVERSATION_DETAIL]` for reload payloads. The sandbox layer now also logs startup image warmup, missing-image pulls, exec/output synchronization, and output-archive entry handling under `[FILE_GENERATE]`, so extend those prefixes instead of inventing new one-off log tags.
