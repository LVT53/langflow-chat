...
function normalizeRerankResponse(
	...
	if (!candidate?.every((item) => isRerankResult(item))) {
...
export async function rerankTexts(params: {
	...
	report({
		...
		confidence:
			normalized.length > 0
				? scoreToConfidencePercent(normalized[0]?.score ?? 0)
				: 0,
	});
	return normalized;
}
...
export async function rerankItems<T>(params: {
		...
		params.onDiagnostics?.({
			...
			confidence:
				rankedItems.length > 0
					? scoreToConfidencePercent(rankedItems[0]?.score ?? 0)
					: 0,
		});
	}
	...
	return {
		...
		confidence:
			rankedItems.length > 0
				? scoreToConfidencePercent(rankedItems[0].score)
				: 0,
	};
}
