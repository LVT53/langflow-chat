...
const FILE_GENERATION_GUARD = [
	...
	"- Use one `produce_file` call per user-facing file request. The tool creates a durable job-backed file card in chat; do not include a file download link in your response text.",
	"- Every call must include `idempotencyKey`, `requestTitle`, `outputs`, `sourceMode`, and `documentIntent`. It may include `templateHint`, `documentSource`, and `program` when relevant.",
	...
	"- `outputs` should be an array like `[{\"type\":\"pdf\"}]`, `[{\"type\":\"docx\"}]`, `[{\"type\":\"html\"}]`, or a multi-output array when the user asks for multiple formats.",
	"- Build `documentSource` as structured content: title, optional subtitle/author/date/locale, and blocks such as headings, paragraphs, lists, tables, callouts, quotes, code, dividers, images, and charts.",
