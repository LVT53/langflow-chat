...
const FILE_GENERATION_GUARD = [
	...
	'- Use `language: "python"` for plain text, CSV, JSON, and HTML files that only need the Python standard library.',
	'- Use `language: "javascript"` for `.xlsx` files with `exceljs`, `.pdf` files with `pdf-lib`, and `.pptx` files with `pptxgenjs`.',
	'- In the JavaScript runtime, write CommonJS-style scripts for `node -e`: use `require(...)`, not top-level `import` statements.',
	'- Do not claim `.xlsx`, `.pdf`, or `.pptx` generation is unavailable when `generate_file` is available. Choose the `javascript` runtime and use the installed libraries directly.',
	'- For `.xlsx`, prefer `exceljs`. For `.pdf`, prefer `pdf-lib`. For `.pptx`, prefer `pptxgenjs`.',
