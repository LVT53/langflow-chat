...
const FILE_GENERATION_GUARD = [
	...
	'- For code/text artifacts, set `requestedOutputs` to the actual requested extension or language, such as `"[{\\"type\\":\\"css\\"}]"`, `"[{\\"type\\":\\"js\\"}]"`, `"[{\\"type\\":\\"ts\\"}]"`, or `"[{\\"type\\":\\"sh\\"}]"`, and set `program.filename` to the exact final filename.',
