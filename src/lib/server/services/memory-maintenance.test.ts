...
const mockState = vi.hoisted(() => {
    ...
    const mockArtifactRows: any[] = [];
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const mockConversationSummaryRows: any[] = [];
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const mockArtifactChunkRows: any[] = [];
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const mockMemoryProjectRows: any[] = [];
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const mockMemoryProjectTaskLinkRows: any[] = [];
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const mockProjectRows: any[] = [];
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const mockConversationRows: any[] = [];
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    let mockPruneOrphanHonchoSessions = vi.fn(async () => ({ deleted: 0, errors: 0 }));
    let mockDeleteOrphanChatFiles = vi.fn(async () => 0);
    let mockFindOrphanFiles = vi.fn(async () => ({ orphanFiles: [], totalFilesOnDisk: 0, totalOrphanBytes: 0 }));
    ...
    function resetMockState() {
        ...
        mockPruneOrphanHonchoSessions = vi.fn(async () => ({ deleted: 0, errors: 0 }));
        mockDeleteOrphanChatFiles = vi.fn(async () => 0);
        mockFindOrphanFiles = vi.fn(async () => ({ orphanFiles: [], totalFilesOnDisk: 0, totalOrphanBytes: 0 }));
    }
...
vi.mock('./semantic-embedding-refresh', () => ({
    backfillSemanticEmbeddingsForUser: vi.fn(async () => ({ artifactCount: 0, taskStateCount: 0 })),
}));

vi.mock('./memory-events', () => ({
    pruneOldMemoryEvents: (...args: unknown[]) => mockState.mockPruneOldMemoryEvents(...args),
}));
...
vi.mock('./disk-reconciliation', () => ({
    findOrphanFiles: (...args: unknown[]) => mockState.mockFindOrphanFiles(...args),
}));
            ...
            from: vi.fn((table: unknown) => {
                ...
                switch (name) {
                    ...
                    case 'conversation_task_states':
                        rows = [{ taskId: 'task-1', status: 'active', updatedAt: new Date() }];
                        break;
...
import { runUserMemoryMaintenance, runAllUsersMemoryMaintenance, ensureMemoryMaintenanceScheduler, stopMemoryMaintenanceScheduler } from './memory-maintenance';
import { repairGeneratedOutputRetrievalClasses, repairGeneratedOutputFamilyStatuses } from './evidence-family';
...
function addArtifactRow(id: string, userId: string, type = 'generated_output') {
    mockState.mockArtifactRows.push({ id, userId, type, name: `artifact-${id}`, updatedAt: new Date(), contentText: 'some content', summary: null });
}
...
function addConversationRow(id: string, userId: string) {
    mockState.mockConversationRows.push({ id, userId, title: 'Test conv', status: 'open' });
}

function addConversationSummaryRow(conversationId: string, userId: string) {
    mockState.mockConversationSummaryRows.push({ conversationId, userId, summary: 'old summary', source: 'deterministic' });
}

function addArtifactChunkRow(id: string, artifactId: string, userId: string) {
    mockState.mockArtifactChunkRows.push({ id, artifactId, userId, chunkIndex: 0, contentText: 'chunk' });
}

function addMemoryProjectRow(projectId: string, userId: string) {
    mockState.mockMemoryProjectRows.push({ projectId, userId, name: `mp-${projectId}`, status: 'active' });
}

function addMemoryProjectTaskLinkRow(id: string, projectId: string, taskId: string, userId: string) {
    mockState.mockMemoryProjectTaskLinkRows.push({ id, projectId, taskId, userId, conversationId: 'conv-1' });
}

function addProjectRow(id: string, userId: string, canonicalMemoryProjectId: string | null = null) {
    mockState.mockProjectRows.push({ id, userId, name: `proj-${id}`, canonicalMemoryProjectId });
}
...
describe('memory-maintenance', () => {
    ...
    describe('performUserMemoryMaintenance — full pipeline', () => {
        ...
        it('calls deleteSemanticEmbeddingsForSubjects for orphan artifacts', async () => {
            ...
            expect(mockState.mockDeleteSemanticEmbeddingsForSubjects).toHaveBeenCalled();
        });
        ...
        it('handles errors in pruneOldMemoryEvents without crashing the pipeline', async () => {
            addUserRow('user-1');
            mockState.mockPruneOldMemoryEvents.mockRejectedValueOnce(new Error('Memory event prune failed'));

            await expect(runUserMemoryMaintenance('user-1', 'manual')).resolves.toBeUndefined();
        });

        it('handles errors in deleteSemanticEmbeddingsForSubjects gracefully', async () => {
            addUserRow('user-1');
            mockState.mockDeleteSemanticEmbeddingsForSubjects.mockRejectedValueOnce(new Error('Semantic delete failed'));
        ...
        it('calls findOrphanFiles and logs results', async () => {
            ...
            mockState.mockFindOrphanFiles.mockResolvedValueOnce({
                orphanFiles: [{ path: 'orphan.txt', sizeBytes: 100, category: 'chat-files' }],
                totalFilesOnDisk: 10,
    ...
    describe('error resilience', () => {
        it('continues after first step failure', async () => {
            addUserRow('user-1');
            vi.mocked(repairGeneratedOutputRetrievalClasses).mockRejectedValueOnce(new Error('repair failed'));
        ...
        it('continues after multiple internal failures', async () => {
            addUserRow('user-1');
            vi.mocked(repairGeneratedOutputRetrievalClasses).mockRejectedValueOnce(new Error('repair 1 failed'));
            mockState.mockPruneOldMemoryEvents.mockRejectedValueOnce(new Error('prune events failed'));
            mockState.mockDeleteSemanticEmbeddingsForSubjects.mockRejectedValueOnce(new Error('delete embeddings failed'));
    ...
    describe('runAllUsersMemoryMaintenance', () => {
        ...
        it('processes zero users gracefully', async () => {
            await expect(runAllUsersMemoryMaintenance('scheduler')).resolves.toBeUndefined();
        });
