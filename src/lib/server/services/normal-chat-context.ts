...
const FILE_GENERATION_GUARD = [
	...
	"- `conversationId` is supplied by the runtime. Do not ask for it and do not include it.",
	"- Required fields: `idempotencyKey`, `requestTitle`, `requestedOutputs`, `sourceMode`, and `documentIntent`.",
	"- For styled documents such as PDF, DOCX, HTML, reports, briefs, and brochures, use `sourceMode: \"document_source\"`.",
	'- Minimal valid `documentSource`: `{"version":1,"template":"alfyai_standard_report","title":"Title","blocks":[{"type":"paragraph","text":"Content."}]}`.',
	"- Add only the document blocks needed to satisfy the request. Prefer headings, paragraphs, bullet lists, and simple tables. Avoid elaborate schemas unless the user explicitly needs them.",
	"- For code/data artifacts such as CSV, JSON, TXT, Markdown, CSS, JS/TS, shell scripts, SVG, ZIP, XLSX, PPTX, or custom packaging, use `sourceMode: \"program\"` and write the final requested files to `/output`.",
	"- For program mode, include `program: { language: \"python\" | \"javascript\", sourceCode, filename }`. Use JavaScript for XLSX/PPTX/DOCX packages that need bundled libraries.",
