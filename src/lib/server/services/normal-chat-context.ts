...
const FILE_GENERATION_GUARD = [
	...
	"- DOCUMENT VERSIONING: When the user asks to update, revise, refine, correct, expand, or create a new version of an existing generated file, reuse the same `filename` (and `requestTitle` or `documentSource.title`) as the previous version. The server automatically versions same-name files (v1 → v2 → v3) so the user can compare versions side-by-side. Only change the filename when the user explicitly asks for a differently named file or a separate document.",
	...
	"",
	"- SURGICAL EDITS WITH PATCHES: When updating an existing text-based file (markdown, txt, etc.), prefer the `patches` field over sending the full file content. The `patches` field lets you make targeted edits — only the changed sections are sent, not the entire file. This is more efficient and produces cleaner version diffs.",
