...
{#if isOpen}
	...
	<div
		use:portal
		class="search-portal-backdrop fixed inset-0 z-[100] flex items-start justify-center p-4 pt-[15vh]"
	...
	>
		<div
			...
			tabindex="-1"
			class="search-portal-modal w-full max-w-[600px] overflow-hidden rounded-lg border border-border bg-surface-overlay shadow-xl"
		...
		>
			<div class="border-b border-border px-4 py-4 md:px-6">
				<div class="flex items-center justify-between gap-4">
					<div class="flex items-center gap-3">
						...
						<h2 id="search-dialog-title" class="text-[18px] font-sans font-semibold text-text-primary">
				...
				<p class="mt-1 pl-[32px] text-[14px] font-sans text-text-muted">
					Find a conversation by title
				</p>
			</div>

			<div class="border-b border-border px-4 py-3 md:px-6">
				<div class="search-input-wrapper flex items-center gap-3 rounded-md border border-border bg-surface-page px-3 py-2">
					...
					{#if searchQuery}
						<button
							type="button"
							class="btn-icon-bare h-7 w-7 text-icon-muted hover:text-icon-primary"
			...
			<div class="max-h-[50vh] overflow-y-auto px-2 py-2">
				{#if searchLoading}
					<div class="flex flex-col items-center justify-center px-4 py-12 text-center">
						<div class="mb-3 flex h-10 w-10 items-center justify-center rounded-full bg-surface-elevated">
							<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="animate-spin text-icon-muted">
								<circle cx="12" cy="12" r="10" stroke-dasharray="32" stroke-dashoffset="12" stroke-linecap="round"></circle>
							</svg>
						</div>
						<h3 class="text-[15px] font-sans text-text-primary">Loading...</h3>
					</div>
				{:else if searchResults.length === 0}
					<div class="flex flex-col items-center justify-center px-4 py-12 text-center">
						...
						<p class="mt-1 text-[13px] font-sans text-text-muted">
				...
				{:else}
					<div class="space-y-1">
						{#each searchResults as conversation (conversation.id)}
							<button
								type="button"
								class="search-result-item flex w-full items-center gap-3 rounded-md px-3 py-2.5 text-left transition-colors duration-150 hover:bg-surface-elevated"
								class:active={conversation.id === $currentConversationId}
								on:click={() => handleSelection(conversation.id)}
							>
								<div class="flex h-8 w-8 shrink-0 items-center justify-center rounded-md bg-surface-elevated">
									<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="text-icon-muted">
										<path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"></path>
									</svg>
								</div>
								<div class="min-w-0 flex-1">
									<div class="truncate text-[14px] font-sans font-medium text-text-primary">
										{conversation.title}
									</div>
									{#if conversation.id === $currentConversationId}
										<div class="mt-0.5 text-[12px] font-sans text-accent">
											Current conversation
										</div>
									{/if}
								</div>
								<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="shrink-0 text-icon-muted">
			...
			<div class="border-t border-border bg-surface-elevated px-4 py-2 md:px-6">
				<div class="flex items-center justify-between text-[12px] font-sans text-text-muted">
					...
					<span class="rounded bg-surface-page px-2 py-0.5">Esc to close</span>
				</div>
			</div>
		</div>
	</div>
