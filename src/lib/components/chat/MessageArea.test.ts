...
describe("MessageArea", () => {
	...
	it("renders a Deep Research card after the user message that triggered it", () => {
		...
		expect(
			getByRole("article", {
				name: "Deep Research: Draft battery recycling research plan",
			}),
		).toBeInTheDocument();
		const renderedText = container.textContent ?? "";
		expect(
			renderedText.indexOf("Please research battery recycling policy."),
		).toBeLessThan(renderedText.indexOf("Research Plan"));
	});
	...
	it("keeps a pre-draft Deep Research card below the user message during trigger id handoff", () => {
		...
		expect(
			renderedText.indexOf("Please research battery recycling policy."),
		).toBeLessThan(renderedText.indexOf("Awaiting plan"));
	});
	...
	it("routes completed Deep Research Report Actions through card callbacks", async () => {
		...
		expect(onResearchFurtherFromDeepResearchReport).toHaveBeenCalledWith(
			"research-job-1",
		);
	});
	...
	it("routes manual Deep Research workflow advancement through the card callback", async () => {
		...
		expect(onAdvanceDeepResearchWorkflow).toHaveBeenCalledWith(
			"research-job-1",
		);
	});
	...
	it("keeps completed Research Cards usable while read-only chat actions are hidden", async () => {
		...
		expect(
			queryByRole("button", { name: "Edit message" }),
		).not.toBeInTheDocument();
		expect(
			queryByRole("button", { name: "Regenerate response" }),
		).not.toBeInTheDocument();
	});
