...
describe("LinkedDocumentPicker", () => {
	it("filters logical documents and returns canonical linked-source identity", async () => {
		...
		await fireEvent.click(
			screen.getByRole("button", { name: "Link selected documents" }),
		);
	...
	it("only offers prompt-ready documents and drops stale selections", () => {
		...
		expect(
			screen.getByRole("checkbox", { name: /Annual report.pdf/i }),
		).toBeInTheDocument();
		expect(screen.queryByText("Still processing.pdf")).toBeNull();
		expect(screen.queryByText("Stale source.pdf")).toBeNull();
	});

	it("matches selected sources against the full Working Document family", async () => {
		const apply = vi.fn();
		render(LinkedDocumentPicker, {
			documents: [
				makeDocument({
	...
	it("shows selected documents and allows removal before applying", async () => {
		...
		const selectedRegion = screen.getByRole("list", {
			name: "Selected linked documents",
		});
		expect(
			selectedRegion.querySelector(".linked-document-picker__selected-chip"),
		).not.toBeNull();
		await fireEvent.click(
			within(selectedRegion).getByRole("button", {
				name: "Remove Annual report.pdf",
			}),
		);
		await fireEvent.click(
			screen.getByRole("button", { name: "Link selected documents" }),
