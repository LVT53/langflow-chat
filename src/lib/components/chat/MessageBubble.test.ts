...
describe("MessageBubble", () => {
	...
	it("removes the preparation status once assistant output surfaces", async () => {
		...
		await rerender({
			message: { ...baseMessage, thinking: "Checking context" },
		});
		expect(screen.queryByText("Preparing response...")).not.toBeInTheDocument();
	...
	it("renders only a compact Max reasoning depth marker above the thinking disclosure", async () => {
		...
		expect(
			screen.queryByTestId("response-activity-timeline"),
		).not.toBeInTheDocument();
		expect(
			screen.queryByText("Reasoning depth selected"),
		).not.toBeInTheDocument();
		...
		for (let index = 0; index < 12; index += 1) {
			expect(
				screen.queryByText(`Context item ${index}`),
			).not.toBeInTheDocument();
		}
	...
	it("shows the compact depth marker for Extended but not Standard or Off", async () => {
		...
		expect(
			screen.queryByTestId("reasoning-depth-indicator"),
		).not.toBeInTheDocument();
	...
	it("does not show a thinking surface for Off turns unless thinking or tool segments actually arrived", () => {
		...
		expect(
			screen.queryByRole("button", { name: /Thinking|Thought/ }),
		).not.toBeInTheDocument();
	});
	...
	it("omits secondary audit details from the info tooltip", () => {
		...
		expect(
			within(tooltip).queryByText("Applied effort"),
		).not.toBeInTheDocument();
		...
		expect(
			within(tooltip).queryByText("Citation audit"),
		).not.toBeInTheDocument();
		...
		expect(
			within(tooltip).queryByText("2 sources reviewed"),
		).not.toBeInTheDocument();
		expect(
	...
	it("renders styled hover tooltip labels for message action icons", () => {
		...
		expect(
			within(assistantContainer).getByRole("tooltip", { name: "Regenerate" }),
		).toBeInTheDocument();
		expect(
			within(assistantContainer).getByRole("tooltip", { name: "Fork" }),
		).toBeInTheDocument();
		expect(
			within(assistantContainer).getByRole("tooltip", { name: "Copy" }),
		).toBeInTheDocument();
		...
		expect(
			within(userContainer).getByRole("tooltip", { name: "Edit" }),
		).toBeInTheDocument();
		expect(
			within(userContainer).getByRole("tooltip", { name: "Copy" }),
		).toBeInTheDocument();
	});
	...
	it("shows a source fork marker with a direct link when one child fork exists", () => {
		...
		expect(
			screen.getByRole("link", { name: "Open fork Source title (fork 1)" }),
		).toHaveAttribute("href", "/chat/fork-1");
	});
	...
	it("shows count-first fork awareness with on-demand links for multiple child forks", async () => {
		...
		const detailsButton = screen.getByRole("button", {
			name: "2 forks from this response",
		});
		...
		expect(
			screen.getByRole("link", { name: "Open fork First fork" }),
		).toHaveAttribute("href", "/chat/fork-1");
		expect(
