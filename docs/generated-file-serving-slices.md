...
**Implementation Status, 2026-05-31:** Finished. `src/lib/server/services/generated-file-serving.ts` now owns the shared generated-file serving contract, chat preview/download routes delegate to it, and Working Document generated-output `sourceChatFileId` serving delegates to it with `displayFilename`.
...
- Current state: `resolveGeneratedFileServing(...)` centralizes generated-file lookup, conversation-owner fallback, assigned-file quarantine, type/byte validation, MIME fallback, and preview/download header policy. Chat routes and Working Document generated-output serving are adapters over that module.
...
- [x] A server-side **Generated File Serving** module exists and owns generated-file lookup, conversation-owner fallback, assigned-file quarantine, allowed-type checks, byte reads, content validation, MIME/content-type resolution, and preview/download headers.
...
- [x] Existing user-visible behavior remains stable for PDFs, Office files, ODT, text/code files, generated HTML, legacy generic MIME generated files, invalid/mismatched files, unreadable content, conversation-owner fallback, and unassigned-file quarantine.
...
**What to build:** Add a focused service module for Generated File Serving. It should accept a user id, chat file id, mode, and optional display filename. It should own user-scoped lookup, conversation-owner fallback, unassigned-file quarantine for public direct serving, allowed-type checks, content reads, `validateGeneratedOutputFile`, preview/download content type selection, and header construction.

**Acceptance criteria**

- [x] Missing files resolve to the same 404 behavior as current chat routes.
- [x] Files with `assistantMessageId === null` stay quarantined from public direct preview/download.
...
Result: 4 files, 31 tests passed.
