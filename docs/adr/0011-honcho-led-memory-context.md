...
**Implementation Status, 2026-05-31:** Knowledge Memory Overview now has a server-owned presentation boundary at `src/lib/server/services/knowledge/memory-overview.ts`. `src/lib/server/services/memory.ts` remains the public service caller and returns the shared `KnowledgeMemorySummary.overviewBullets` contract with status/source metadata. The Knowledge page and `KnowledgeMemoryView.svelte` render that contract directly instead of cleaning raw Honcho text in route-local helpers. This preserves Honcho as persona/memory authority while keeping user-facing overview shaping out of the UI.
