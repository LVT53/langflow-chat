...
- The live document-state signals are assembled through one shared server helper instead of being recomputed separately in working-set refresh and task evidence selection. Active workspace focus, current generated-output selection, recent document-family refinement, and user-correction phrasing should now stay in sync across those paths.
