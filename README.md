...
- Wave 4 active-state inference has started with an explicit user-correction signal. When the user is clearly revising a document or version (`actually`, `instead`, `use the previous version`, etc.), working-set and prompt selection now keep the focused document live even if the semantic match is weak.
- The live document-state signals are now assembled through one shared server helper instead of being recomputed separately in working-set refresh and task evidence selection. Active workspace focus, current generated-output selection, and user-correction phrasing should now stay in sync across those paths.
