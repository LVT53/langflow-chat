...
- TEI retrieval planning now lives in [docs/tei-retrieval-roadmap.md](./docs/tei-retrieval-roadmap.md). The current integration wave only adds runtime config plus thin TEI clients; semantic retrieval/reranking rollout should continue wave-by-wave from that document instead of bypassing deterministic context and memory rules.
- Wave 4 has now started: the current evidence-rerank, chunk-rerank, historical-section-rerank, and tool/web-evidence-rerank paths use the TEI reranker while preserving the existing deterministic caps and fallback-to-current-order behavior when reranking is unavailable.
- Wave 2 now stores embeddings durably but does not yet change retrieval behavior by itself. Semantic shortlist retrieval for documents, persona memory, and task continuity will layer on top of the shared `semantic_embeddings` store in later waves.
