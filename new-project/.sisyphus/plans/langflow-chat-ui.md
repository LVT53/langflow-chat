...
> 
> **Estimated Effort**: Large (12 atomic commits, ~40 tasks)
> **Parallel Execution**: YES — 5 waves
> **Critical Path**: Task 1 → Task 3 → Task 6 → Task 12 → Task 18 → Task 22 → Task 28 → Task 33 → Final
...
Critical Path: Task 1 → Task 3 → Task 7 → Task 12 → Task 18 → Task 20 → Task 28 → Task 33 → F1-F4
...
- [ ] 1. Project Scaffolding — SvelteKit + TypeScript + Tailwind + Vitest + Playwright
...
- [ ] 3. SQLite Schema with Drizzle ORM
...
- [ ] 4. Shared TypeScript Types and Interfaces
...
- [ ] 5. Mock Langflow Server + Mock nemotron-nano Server
...
- [ ] 7. Authentication API Routes (Login / Logout)
...
- [ ] 10. App Shell Layout (Sidebar + Chat Area + Header)
...
- [ ] 11. Dark/Light Mode Toggle with Persistence
...
- [ ] 12. Langflow API Client Service
...
- [ ] 13. Conversation CRUD Service + API Routes
...
- [ ] 14. Conversation Sidebar Component
...
- [ ] 16. Markdown Rendering Engine (marked + DOMPurify + Shiki)
...
- [ ] 17. Message Display Component (with Copy Buttons)
...
- [ ] 18. Non-Streaming Chat Flow (Send → Wait → Display Response)
...
- [ ] 19. Loading/Status Indicator Component
...
- [ ] 20. English SSE Streaming — Server Proxy Endpoint
...
- [ ] 21. English SSE Streaming — Client-Side Consumer + Display
...
- [ ] 22. Streaming Markdown Renderer (Incremental Token Display)
...
- [ ] 23. Title Generation Service (nemotron-nano)
...
- [ ] 24. Title Generation Integration (Fire-and-Forget After First Response)
...
- [ ] 25. Hungarian Non-Streaming Path (Full Response Wait)
...
- [ ] 26. File Upload Placeholder Button
...
- [ ] 27. Responsive Layout (Sidebar Collapse, Tablet Support)
...
- [ ] 28. Hungarian Webhook Receiver Endpoint
...
- [ ] 29. Hungarian Webhook → Browser SSE Bridge
...
- [ ] 30. Unified Streaming Abstraction (SSE + Webhook → Same Display)
...
- [ ] 31. Error Handling (Timeouts, Retries, Network Errors)
...
- [ ] 32. Conversation Delete + Rename UI
...
- [ ] 33. Apache VirtualHost Configuration + SSE Anti-Buffering
...
- [ ] 34. systemd Service File + Deployment Documentation
...
- [ ] 35. Playwright E2E Test Suite (Full User Journey)
...
> **STATIC CHECKS ONLY** — No runtime test execution. Tests verified to exist and compile.

- [ ] F1. **Plan Compliance Audit** — `oracle`
  Read the plan end-to-end. For each "Must Have": verify implementation exists (read file, check exports, validate structure). For each "Must NOT Have": search codebase for forbidden patterns — reject with file:line if found. Verify all task deliverables exist as files. Compare against plan requirements.
  Output: `Must Have [N/N] | Must NOT Have [N/N] | Tasks [N/N] | VERDICT: APPROVE/REJECT`

- [ ] F2. **Code Quality Review** — `unspecified-high`
  Run `npx tsc --noEmit` + `npm run lint` + `npm run build`. Verify test files exist in `src/**/*.test.ts` and `tests/e2e/**/*.spec.ts`. Review all changed files for: `as any`/`@ts-ignore`, empty catches, console.log in prod, commented-out code, unused imports. Check AI slop: excessive comments, over-abstraction, generic names (data/result/item/temp). Verify all `.env.example` vars are documented. **Do NOT run `npm test` or `npx playwright test`** — runtime tests are for manual execution on deployment machine.
  Output: `Build [PASS/FAIL] | Lint [PASS/FAIL] | Test Files [N exist] | Files [N clean/N issues] | VERDICT`

- [ ] F3. **Test Suite Verification** — `unspecified-high`
  Verify all test files exist and are syntactically valid (TypeScript compiles). Check test coverage by reviewing test file contents — ensure each feature has corresponding tests. Verify mock servers exist in `tests/mocks/`. Verify Playwright config and E2E test structure. Document test execution commands for user's reference. **Do NOT execute runtime tests** — they will be run manually on deployment machine.
  Output: `Vitest Files [N exist] | Playwright Files [N exist] | Mocks [N exist] | Config [VALID] | VERDICT`

- [ ] F4. **Scope Fidelity Check** — `deep`
  For each task: read "What to do", read actual diff (git log/diff). Verify 1:1 — everything in spec was built (no missing), nothing beyond spec was built (no creep). Check "Must NOT Have" compliance: no message content in SQLite, no file upload logic, no export, no WebSocket, no admin UI, no auth libraries. Detect unaccounted changes.
...
> **Pre-commit checks are STATIC ONLY**: `npm run build`, `npx tsc --noEmit`, `npm run lint`
> Runtime tests (`npm test`, `npx playwright test`) are NOT run by agents.
...
### Agent Verification Commands (Static Checks)
```bash
...
- [ ] `npm run lint` passes
- [ ] All vitest test files exist and compile
- [ ] All Playwright E2E test files exist and compile
